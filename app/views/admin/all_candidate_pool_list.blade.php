...
<head>
    ...
    <title>TalentPool - Admin - Candidate Pool List</title>
    ...
    <div class="row">
        <div class="col-sm-12">
            <section class="panel">
                ...
                <tbody>
                @foreach( $candidatePool as $candidatePoollist )
                
                <tr class="gradeA">
                    <td>{{ $candidatePoollist->name }}</td>
                   

                    @if($candidatePoollist->user_id != 0 || $candidatePoollist->user_id == "")
                    <td>{{ $candidatePoollist->user->first_name }}</td>
                    @else
                        <td></td>
                    @endif
                     <td>{{ date("l jS F, Y",strtotime($candidatePoollist->created_at)) }}</td>
                </tr>
                @endforeach
                </tbody>
                <tfoot>
